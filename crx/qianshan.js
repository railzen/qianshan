$(document).ready(function(){
  ...
  $("a.website").click(function(e){
    ...
    e.preventDefault();
    if (self.hasClass("navigating")){
      return false;
    }
    self.removeClass("hover");
    self.addClass("clicked navigating");
    setTimeout(function(){
  ...
  //initiateEventListenerForWebsiteInMoreWebsiteGrid，首页更多站点的事件监听器，不需要展示预览，不需要修改编辑框的内容，直接跳转站点即可
  function initiateEventListenerForWebsiteInMoreWebsiteGrid(){
    ...
    $('div#more-website-grid li.website span.website').on('click', function(){
      var websiteSerialIDInArray = $(this).attr('id');
      openWebsiteInCurrentTab(websiteSerialIDInArray);
      $(this).removeClass('hover');
    });
  }
  //openWebsiteInCurrentTab，更多站点弹层点击站点的后继事件，从当前站点序列中提取出url，并在当前标签页中打开
  function openWebsiteInCurrentTab(websiteSerialIDInArray){
    var website = new Object();
    website = currentWebsiteChoicesInArray[parseInt(websiteSerialIDInArray)];
    var website_url = website.url;
  ...
  function hotkeyListener(){
    ...
    for(var i = 13; i < 91; i++){
      if (i == k){
        ...
        if(obj.attr("href") != undefined){
          obj.addClass("hover");
          var website_href = obj.attr("href");
          setTimeout(function(){
            obj.removeClass("hover");
            window.location.href = website_href;
          },500);
        }
        break;
      }
